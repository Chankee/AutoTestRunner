...
from .compat import basestring, numeric_types, str
...
def __prepare_testcase_tests(tests, config, project_mapping, session_variables_set=None):
    ...
    for test_dict in tests:
        ...
        # # 获取request片段中的json部分，并添加到变量列表中
        # # 执行testsuites目录或testcases目录中的用例
        # if "api_def" in test_dict:
        #     api_def = test_dict["api_def"]
        #     if '$json' in list(api_def["variables"].values()) and "json" in api_def["request"]:
        #         api_def["variables"]['json'] = json.dumps(api_def["request"]["json"])
        #     elif '$json' in list(api_def["variables"].values()) and "json" not in api_def["request"]:
        #         raise Exception("api的request片段中没有json关键字，variables中不允许出现$json")
        # # 执行api目录下的用例
        # else:
        #     if '$json' in list(test_dict["variables"].values()) and "json" in test_dict["request"]:
        #         test_dict["variables"]['json'] = json.dumps(test_dict["request"]["json"])
        #     elif '$json' in list(test_dict["variables"].values()) and "json" not in test_dict["request"]:
        #         raise Exception("api的request片段中没有json关键字，variables中不允许出现$json")
        #     pass
...
def _parse_testcase(testcase, project_mapping, session_variables_set=None):
    ...
    """
    print (testcase, project_mapping,'\n','------------')
