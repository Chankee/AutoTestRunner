...
# 用例集反序列化验证器
class TestSuiteSearchSerializer(serializers.Serializer):
    node = serializers.IntegerField(min_value=0, default='')
    project = serializers.IntegerField(required=True, min_value=1)
    search = serializers.CharField(default='')
...
class TestSuiteStepSerializer(serializers.ModelSerializer):
    """
    用例步骤序列化
    """
    ...
    class Meta:
        model = models.TestSuiteStep
        fields = ['id', 'name', 'url',  'case', 'creator', 'updater']
        depth = 1
