...
import yaml
from . import built_in, exceptions, logger, parser, utils, validator
...
def load_yaml_file(yaml_file):
    ...
    with io.open(yaml_file, 'r', encoding='utf-8') as stream:
        yaml_content = yaml.load(stream,yaml.FullLoader)
...
def load_python_module(module):
    ...
    Returns:0     dict: variables and functions mapping for specified python module
...
def _load_teststeps(test_block, project_mapping):
    """ load teststeps with apil/testcase references
...
def _load_testcase(raw_testcase, project_mapping):
    """ load testcase/testsuite with apil/testcase references
...
def _get_block_by_name(ref_call, ref_type, project_mapping):
    ...
    Args:
        ...
        ref_type (enum): "def-apil" or "def-testcase"
        project_mapping (dict): project_mapping

    Returns:
        dict: apil/testcase definition.
...
def _get_test_definition(name, ref_type, project_mapping):
    """ get expected apil or testcase.

    Args:
        name (str): apil or testcase name
        ref_type (enum): "def-apil" or "def-testcase"
        project_mapping (dict): project_mapping

    Returns:
        dict: expected apil/testcase info if found.

    Raises:
        exceptions.ApiNotFound: apil not found
...
def _extend_block(ref_block, def_block):
    ...
    Args:
        def_block (dict): apil definition dict.
...
def load_folder_content(folder_path):
    """ load apil/testcases/testsuites definitions from folder.

    Args:
        folder_path (str): apil/testcases/testsuites files folder.

    Returns:
        dict: apil definition mapping.

            {
                "tests/apil/basic.yml": [
                    {"apil": {"def": "api_login", "request": {}, "validate": []}},
                    {"apil": {"def": "api_logout", "request": {}, "validate": []}}
                ]
            }
...
def load_api_folder(api_folder_path):
    """ load apil definitions from apil folder.

    Args:
        api_folder_path (str): apil files folder.

            apil file should be in the following format:
            [
                {
                    "apil": {
                        "def": "api_login",
...
def load_project_tests(test_path, dot_env_path=None):
    """ load apil, testcases, .env, builtin module and debugtalk.py.
        apil/testcases folder is relative to project_working_directory
    ...
    Returns:
        dict: project loaded apil/testcases definitions, environments and debugtalk.py module.
...
def load_tests(path, dot_env_path=None):
    """ load testcases from file path, extend and merge with apil/testcase definitions.
